...
class ProfileView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        profile_data = GetProfileSerializer(request.user.profile.get()).data
        return Response(data=profile_data, status=status.HTTP_200_OK)
    ...
    def put(self, request):
        ...
        # formData.pop('user')
        formData['user'] = request.user.id

        instance = request.user.profile.get()
        serializer = ProfileSerializer(instance, data=formData)
...
class SingleProfileView(APIView):
    def get(self, request, *args, **kwargs):
        try:
            ...
            profile_data = GetProfileSerializer(user.profile.get()).data
            return Response(data=profile_data, status=status.HTTP_200_OK)
